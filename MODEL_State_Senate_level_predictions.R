...
library(readr)
#Read BLW DATA: BLWatDist <- read_sf("endistptsID.shp")
dat <- BLWatDist
...
table(dat$OBJECTID_1)
dat$disID <- as.character(dat$OBJECTID_1)
dat$NAME <- dat$disID
table(dat$NAME)
...
#Read Congress data from . zip congdat <- read_sf("StateSenateLevelData.shp")
...
fdat$TRUMP <- fdat$RepPct

fdat$NAME <- as.character(fdat$NAME)
...
Census <- hhidat
table(Census$OBJECTID_1)
sen_ranefs <- array(NA, c(1939, 1))
dimnames(sen_ranefs) <- list(c(Census$OBJECTID_1), 'effect')
